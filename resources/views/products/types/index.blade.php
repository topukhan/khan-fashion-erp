<x-ui.master>
    <div class="container mx-auto p-4">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-bold text-gray-800">Types</h1>
            <a href="{{ route('types.create') }}" class="bg-green-500 text-white px-4 py-2 rounded-md">Create
                Product Type</a>
        </div>
        <div class="my-3">
            @if (session('message'))
                ...
                <tbody>
                    @foreach ($types as $key => $type)
                        <tr class="border-b text-center">
                            ...
                            <td class="py-4 px-6 border-gray-200">{{ $type->name }}</td>
                            <td class="py-4 px-6 border-gray-200">
                                ...
                                <form method="POST" action="{{ route('types.destroy', $type->id) }}" class="inline">
                                    @csrf
                                    @method('delete')
                                    <button type="submit"
                                        onclick="return confirm('Are you want to delete this type?')"
